...
class _NormBase(Module):
    ...
    def __init__(
        ...
        if self.track_running_stats:
            ...
            self.register_parameter('train_running_mean', None)
            self.register_parameter('train_running_var', None)
    ...
    def reset_running_stats(self) -> None:
        if self.track_running_stats:
            ...
            # self.train_running_mean.zero_()
            # self.train_running_var.fill_(1)
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        if self.training:
            ...
            F.batch_norm(
                ...
                self.running_mean if not self.training or self.track_running_stats else None,
                self.running_var if not self.training or self.track_running_stats else None,
                self.weight, self.bias, bn_training, self.momentum, self.eps)
            
            bn_training = False
            return F.batch_norm(
                input,
                # If buffers are not to be tracked, ensure that they won't be updated
                self.train_running_mean,
                self.train_running_var,
                self.weight, self.bias, bn_training, self.train_momentum, self.eps)
        else:
            bn_training = (self.running_mean is None) and (self.running_var is None)
            return F.batch_norm(
                input,
