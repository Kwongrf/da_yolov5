...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        # for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # ridx = random.randint(4, batch_size // 2)
            # print(ridx)
            # ridx = 
            imgs = torch.cat((imgs, t_imgs), 0)
            targets = targets
            ...
            dloss_inst1, dloss_inst2, dloss_inst3, dloss_s1, dloss_s2, dloss_s3 = 1e-8, 1e-8, 1e-8, 1e-8, 1e-8, 1e-8
            inst_weight = 0.5
            with torch.autograd.set_detect_anomaly(True):
                with amp.autocast(enabled=cuda):
                    ...
                    # domain label
                    domain_s2 = domain_s3 = Variable(torch.cat((torch.zeros(batch_size).long().cuda(), \
                        torch.ones(batch_size).long().cuda()), 0))
                    ...
                    # k=1th loss
                    dloss_s1 = 0.5 * (0.5 * torch.mean(out_d1[:batch_size] ** 2) + 0.5 * torch.mean((1 - out_d1[batch_size:]) ** 2))
                    # k=2nd loss
                    dloss_s2 = 0.5 * nn.CrossEntropyLoss()(out_d2, domain_s2) * 0.15
                    # k = 3rd loss 
                    dloss_s3 = 0.5 * FocalLoss(2)(out_d3, domain_s3)
                    # loss, loss_items = compute_loss(pred, targets.to(device))  # loss scaled by batch_size
                    loss, loss_items = compute_loss(pred[:batch_size], targets.to(device))  # loss scaled by batch_size
                    if rank != -1:
                ...
                if opt.da == 8:
                    loss += (dloss_s1 + dloss_s2 + dloss_s3 )
        ...
        if rank in [-1, 0]:
            ...
            tags = ['train/box_loss', 'train/obj_loss', 'train/cls_loss',  # train loss
                    ...
                    'x/lr0', 'x/lr1', 'x/lr2', \
                    'train/dloss_s1','train/dloss_s2', 'train/dloss_s3',\
